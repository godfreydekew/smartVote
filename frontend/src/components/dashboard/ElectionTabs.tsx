...
export const ElectionTabs = memo(() => {
  ...
  useEffect(() => {
    const fetchElectionsWithMetadata = async () => {
      try {
        ...
        const formatElections = (
          contractData: ElectionData[] | undefined,
          status: 'active' | 'upcoming' | 'completed'
        ) => {
    ...
    if (
      (isAdmin && ownerElections) ||
      (!isAdmin && activeElectionsData && upcomingElectionsData && completedElectionsData)
    ) {
  ...
  const tabOptions = [
    { value: 'active', label: t('dashboard.activeElections'), icon: Activity, count: activeElections.length },
    { value: 'upcoming', label: t('dashboard.upcomingElections'), icon: Calendar, count: upcomingElections.length },
    {
      value: 'completed',
      label: t('dashboard.completedElections'),
      icon: CheckCheck,
  ...
  if (
    isLoading ||
    (!isAdmin && (isActivePending || isUpcomingPending || isCompletedPending)) ||
    (isAdmin && isOwnerElections)
  ) {
    return (
      <div className="flex justify-center items-center h-full">
        ...
        <span>{t('dashboard.loadingElections')}</span>
      </div>
    );
  }
        ...
        <div className="px-4">
          <Select value={activeTab} onValueChange={setActiveTab}>
            <SelectTrigger className="w-full bg-white">
              <SelectValue>{tabOptions.find((option) => option.value === activeTab)?.label}</SelectValue>
            </SelectTrigger>
            <SelectContent>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <TabsContent value="active">
            <ElectionsGrid elections={activeElections} status="active" />
          </TabsContent>
          <TabsContent value="upcoming">
            <ElectionsGrid elections={upcomingElections} status="upcoming" />
          </TabsContent>
          <TabsContent value="completed">
            <ElectionsGrid elections={completedElections} status="completed" />
          </TabsContent>
        </motion.div>
      </AnimatePresence>
    </Tabs>
