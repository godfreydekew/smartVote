  ...
  return (
    ...
    >
      <div className="aspect-video relative">
        ...
        {candidate.winner && (
          <div className="absolute top-0 left-0 bg-green-500 text-white px-3 py-1 text-xs font-medium">Winner</div>
        )}
      </div>
      ...
      <CardContent className="p-4">
        ...
        {candidate.party && <p className="text-gray-500 text-sm">{candidate.party}</p>}
          ...
          <TooltipProvider>
            ...
            {candidate.website && (
              <Tooltip>
                ...
                <TooltipContent>
                  <p>{t('electionDetails.candidates.officialWebsite')}</p>
                </TooltipContent>
              </Tooltip>
            )}
          </TooltipProvider>
        ...
        {isCompleted && !isPending && (
          <p className="mt-2 font-medium">
            {t('electionDetails.candidates.receivedVotes', { count: candidate.voteCount })}
        ...
        <div className="mt-4 flex gap-2">
          <Button variant="outline" className="w-full" onClick={() => onViewProfile(candidate)}>
            {t('electionDetails.candidates.viewProfile')}
          </Button>

          {isActive && (
            <Button className="w-full" onClick={() => onVote(candidate)}>
              {t('electionDetails.candidates.vote')}
            </Button>
          )}
        </div>
      </CardContent>
