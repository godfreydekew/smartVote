...
export const ElectionStats: React.FC<ElectionStatsProps> = ({
  address
}) => {
  ...
  const { data: electionData, isPending: isElectionPending } = useReadContract({
    contract: electionContract,
    method: "function election() view returns (uint256 id, string title, uint256 startTime, uint256 endTime, uint8 state, bool isPublic, address owner, uint256 totalVotes, bool exists)",
    params: [],
  });

  // Transform the data for the pie chart
  const chartData = sortedResults?.map((result: CandidateResult) => ({
    name: result.name,
    value: Number(result.voteCount),
    percentage: electionData && Number(electionData[7]) > 0 
      ...
      return (
        ...
        >
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-gray-600">
              <span className="font-medium">Rank:</span> {chartData.findIndex(item => item.name === payload[0].payload.name) + 1}
            </p>
          </div>
        </motion.div>
      );
          ...
          <div className="flex items-center gap-2">
            <TooltipProvider>
              <UITooltip>
                ...
                <TooltipContent>
                  <p className="max-w-xs">Visual representation of votes distributed among candidates. Hover over segments for detailed information.</p>
                </TooltipContent>
              </UITooltip>
            </TooltipProvider>
          </div>
