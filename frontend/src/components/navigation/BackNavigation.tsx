...
const getContextLabel = (pathname: string, t: any): string => {
  // Extract page context for dynamic labels
  if (pathname.includes('/election/')) {
    return t('adminDashboard.backNavigation.elections');
  } else if (pathname.includes('/admin/')) {
    return t('adminDashboard.backNavigation.adminDashboard');
  } else if (pathname === '/profile') {
    return t('adminDashboard.backNavigation.dashboard');
  } else {
    return t('adminDashboard.backNavigation.dashboard');
  }
};

export function BackNavigation({ to, label, className, fixed = false }: BackNavigationProps) {
  ...
  const contextLabel = getContextLabel(location.pathname, t);
